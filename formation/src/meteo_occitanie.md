...
const nomStation = view(Inputs.select(nomsStations, {label: "Choisir une station météo"}));
```

```js
// Filtrage des données avec le résultat de l'input select
const donneesChoisies = meteo_31_2023_2024_extrait.filter(d => d.nom_poste === nomStation);
